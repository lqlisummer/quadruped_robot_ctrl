...
constexpr const char* kDefaultConfigPath = "stages/stage1_single_motor/config/motor_cli.conf";
...
bool parseMode(const std::string& text, MotorControlMode& mode) {
    if (text == "position") {
        ...
        mode = MotorControlMode::Current;
    } else if (text == "pd") {
        mode = MotorControlMode::Pd;
    } else if (text == "brake") {
        mode = MotorControlMode::Brake;
    } else if (text == "openloop") {
        mode = MotorControlMode::OpenLoop;
    } else {
        return false;
    }
    return true;
...
bool parseTheta(const std::string& text, ThetaSource& source) {
    if (text == "force") {
        source = ThetaSource::ForceTheta;
    } else if (text == "sensorless") {
        source = ThetaSource::SensorLess;
    } else if (text == "sensor") {
        source = ThetaSource::Sensor;
    } else if (text == "fusion") {
        source = ThetaSource::SensorFusion;
    } else {
        return false;
    }
    return true;
...
void printUsage() {
    std::cout
        << "qrc_stage1_single_motor --config <path> <command> [args]\n\n"
        << "Commands:\n"
        << "  dump-config\n"
        << "  raw-get-model\n"
        << "  info\n"
...
bool runRawGetModel(const BoardConfig& board_cfg, const MotorConfig& motor_cfg, std::string& model, std::string* board_fw) {
    BoardClient board(board_cfg);
    if (!board.connect()) {
        return false;
    }

    MotorDriver driver(board, motor_cfg);
    if (!driver.create()) {
        board.disconnect();
        return false;
    }

    model = driver.queryMotorModel();
    if (board_fw != nullptr) {
        *board_fw = board.queryFirmwareVersion();
    }
...
int runShell(SingleMotorController& controller, const BoardConfig& board_cfg, const MotorConfig& motor_cfg) {
    ...
    while (std::cout << "> " && std::getline(std::cin, line)) {
        ...
        if (cmd == "info") {
            const auto id = controller.identify();
        ...
        if (cmd == "enable") {
            std::cout << (controller.enable() ? "ok\n" : "failed\n");
            continue;
        }
        if (cmd == "disable") {
            std::cout << (controller.disable() ? "ok\n" : "failed\n");
            continue;
        }
        if (cmd == "clear-fault") {
            std::cout << (controller.clearFault() ? "ok\n" : "failed\n");
            continue;
        }
        if (cmd == "zero") {
            std::cout << (controller.zero() ? "ok\n" : "failed\n");
            continue;
        }
        if (cmd == "calibrate") {
            std::cout << (controller.calibrate() ? "ok\n" : "failed\n");
            continue;
        }
        ...
        if (cmd == "strong-drag" && parts.size() >= 3) {
            ThetaSource source{};
            if (!parseTheta(parts[1], source)) {
                std::cout << "invalid strong-drag source\n";
                continue;
            }
            controller.setMode(MotorControlMode::Velocity);
            controller.enable();
...
int CliApp::run(int argc, char** argv) {
    ...
    if (!bootstrapController(controller)) {
        ...
        std::cerr << "hint: build on the Linux control host with -DQRC_ENABLE_PHYARC_SDK=ON and verify board network settings\n";
        return 3;
    }
    ...
    if (cmd == "set-pos") {
        ...
        if (!controller.setMode(MotorControlMode::Position) || !controller.enable()) {
    ...
    if (cmd == "set-vel") {
        ...
        if (!controller.setMode(MotorControlMode::Velocity) || !controller.enable()) {
            return printCommandResult(false);
        }
    ...
    if (cmd == "set-cur") {
        ...
        if (!controller.setMode(MotorControlMode::Current) || !controller.enable()) {
            return printCommandResult(false);
        }
    ...
    if (cmd == "set-pd-target") {
        ...
        if (!controller.setMode(MotorControlMode::Pd) || !controller.enable()) {
            return printCommandResult(false);
        }
        controller.driver().setPd(motor_cfg.pd_kp, motor_cfg.pd_kd);
